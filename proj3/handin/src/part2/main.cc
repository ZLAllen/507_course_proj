 ...
 */
    ...
    os << endl << endl;
    for(int i = 0; i < P; i++){
        os << "   datapath #(M,N,T,P) d_" << to_string(i) << "(clk, clear_acc, data_out_x, data_out_" << to_string(i) << ", data_out_w_" << to_string(i) << ", data_out_b_" << to_string(i) << ", wr_en_y, m_valid, m_ready, sel); " << endl;
    }
    os << "   ctrlpath #(M,N,T,P) c(clk, reset, s_valid, s_ready, m_valid, m_ready, clear_acc,wr_en_x, wr_en_y, sel, addr_x";
    for(int i = 0; i < P; i++){
   ...
   // os << "         default : data_out <= data_out_" << to_string(P-1) << ";" << endl;
    ...
    os << "endmodule " << endl;
    os << " " << endl;
    os << "module datapath(clk, clear_acc, data_out_x, data_out, data_out_w, data_out_b, wr_en_y, m_valid, m_ready, sel); " << endl;
    os << " " << endl;
    ...
    os << "endmodule " << endl;
    os << " " << endl;
    os << "module ctrlpath(clk, reset, s_valid, s_ready, m_valid, m_ready, clear_acc,wr_en_x, wr_en_y, sel, addr_x";
    for(int i = 0; i < P; i++){
        os << ", addr_w_" << to_string(i) << ", addr_b_" << to_string(i);
    }
    os << ");" << endl;
