...
void genAllLayers(int N, int M1, int M2, int M3, int mult_budget, int bits, vector<int>& constVector, string modName, ofstream &os) {
    ...
    * the latency is given by (N + 1 + N + 1) * M cycles. The first N is input cycles, followed by 1 cycle of state transition to reset address counter for 
    * calculation. Then N cycles of calculation for each row and 1 cycle of forwarding data to output. For example, a 3 by 2 matrix will takes 6 * 3 = 18 
    ...
    * be given by [(2N+2)*M/P] (M/P should be valid integer).
    * */
    
    // initially, all parallelism are 1, minimum multiplier cost is 3.
   vector<long> f;
   f.push_back((2*N + 2) * M1);
   f.push_back((2*M1 + 2) * M2);
   f.push_back((2*M2 + 2) * M3);
   int cost = 3;
   ...
   while(!done){
       for(auto it = cycle.rbegin(); it != cycle.rend(); it++){
           ...
           if(cost + panini <= mult_budget){
               P[index]++;
               cycle.erase(key);
               cycle[f[index]/factor] = index;
               cout << "special first= " << f[index]/factor << " second= " << index << endl;
               cost += panini;
               break;
           }else{
               auto endIt = cycle.rend();
